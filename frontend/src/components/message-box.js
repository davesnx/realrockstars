...
// import constants { unit, colors, areas, borderRadiuses, innerSpaces } from './constants'
...
const MessageCard = styled.div`
  ...
  ${props =>
    props.maxWidth === 'xlg' &&
    `max-width: ${messageMaxWidths[props.maxWidth]}`};
  ${constants.borderRadiuses.sm};
  ${constants.innerSpaces.sm};
`

const MessageBox = ({ type, maxWidth, title, description, ...rest }) => (
  <MessageCard type={type} maxWidth={maxWidth}>
    {title && (
      <Spacer bottom={1}>
        <Text type={type} fontWeight="bold">
          {title}
        </Text>
      </Spacer>
    )}
